...
<p>As you can see, I used simple HTML markup, including the bold and horizontal line tags to add style elements. Each instance of {{ TEXTHER }}} corresponds to a column name from the shapefile, and calls it up in the tooltip. So, to call up the percent change, I typed {{ PERCENT }}, which refers to the PERCENT column from our data. Upon saving this, you'll see that the tooltip appears however you format it. The possibilities here really abound.</p>
